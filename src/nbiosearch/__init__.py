...
class Finger:
    ...
    def set_payload(self, text):
        ...
        """
        self.payload = text
        self.FIR = _bsp_search.payload(self.__handler, self.FIR, text)
    ...
    def text(self):
        ...
        """
        return FingerText(_bsp_search.text_fir(self.__handler, self.FIR))
    ...
    def __del__(self):
        del(self.buffer)
        _bsp_search.free_fir(self.__handler, self.FIR)
...
class Handler:
    ...
    def verify(self, finger1, finger2, timeout=5):
        ...
        match, payload = _bsp_search.verify(self._handler, finger2, finger1)
        return match == 1 and (payload or match) or 0

    def close(self):
        """Close the currently opened Nitgen device
        """
        _bsp_search.close(self._handler)
...
class SearchEngine:
    ...
    def identify(self, finger, security_level=5, timeout=5):
